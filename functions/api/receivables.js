import { fetchReputationMap, getSupabaseConfig, handleOptions, jsonResponse, rowToInvoice, supabaseHeaders } from "../_shared/receivables.js";
...
export async function onRequestGet({ env }) {
  try {
    ...
    const reputations = await fetchReputationMap(env);
    return jsonResponse(rows.map((row) => {
      const invoice = rowToInvoice(row);
      return {
        ...invoice,
        issuerReputation: reputations.get(invoice.issuer.toLowerCase()),
        payerReputation: reputations.get(invoice.payer.toLowerCase()),
        buyerReputation: invoice.buyer ? reputations.get(invoice.buyer.toLowerCase()) : undefined,
      };
    }));
  } catch (error) {
    console.error(error);
